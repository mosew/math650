...
### FREQUENCY PLOTS OF SOME VARIABLES
plot.br = function(name){
  ind=which(names(X)==paste(name))
  p=ggplot()+geom_freqpoly(mapping=aes(x=X[,ind][X$rpct>50]),bins=50,color="red")+
    geom_freqpoly(aes(x=X[,ind][X$rpct<50]),color="blue",bins = 50)+
    labs(title=c(paste(name)," vs count"),x="",y="number of counties")
  p
}


grid.arrange(plot.br("voterparticip"),
             plot.br("lat"),
             plot.br("lnpoppersqmi"),
             plot.br("married"),
             plot.br("homeaffordability"),
             plot.br("vehperhouse"),
             plot.br("flow10yr"),
             plot.br("poverty"),
             plot.br("lnpci"),
